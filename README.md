...
| `TARGET_REGION` | 目标地区（支持多地区，详见下方说明） | `广州` 或 `广州.深圳.北京` |
...
`TARGET_REGION` 支持使用以下分隔符同时配置多个地区：
- `.`（点号）
- `\`（反斜杠）
- `-`（短横线）
...
**投递结果：**
- 每个地区会独立完成投递流程
- 完成后会收到类似 `广州地区投递完成,深圳地区投递完成,北京地区的职业过少` 的邮件通知
- 如果某个地区岗位过少，会标记为「地区的职业过少」
